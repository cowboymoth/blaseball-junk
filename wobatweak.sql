...
player_woba as (
	...
	group by p.player_name, ge.season
	having count(*) filter (where ge.event_type in (select event_type from taxa.event_types where plate_appearance = 1)) >= 99
	order by wOBA desc
)
-- List requested data
select *
